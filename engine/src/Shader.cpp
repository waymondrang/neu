...
Shader::Shader(const string &vertexShaderPath, const string &fragmentShaderPath)
{
    ...
    Logger::debug("Found %d %s in shader \"%s\"\n", numBlocks, numBlocks == 1 ? "block" : "blocks", name.c_str());
    ...
    // printf("[INFO] Created shader program (%d) with vertex shader \"%s\" and fragment shader \"%s\"\n", ID, vertexShaderPath.c_str(), fragmentShaderPath.c_str());
    Logger::info("Created shader program (%d) with vertex shader \"%s\" and fragment shader \"%s\"\n", ID, vertexShaderPath.c_str(), fragmentShaderPath.c_str());
}

void Shader::attachShader(const string &shaderPath, unsigned int shaderType)
{
    string shaderCode;
    ifstream shaderFile;
...
void Shader::bindUniformBlock(const string &blockName, unsigned int bindingPoint)
{
    unsigned int blockIndex = glGetUniformBlockIndex(ID, blockName.c_str());

    Logger::info("Binding uniform block \"%s\" to binding point %d (block index: %d) in shader \"%s\"\n", blockName.c_str(), bindingPoint, blockIndex, name.c_str());
