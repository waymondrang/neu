...
void InputManager::registerMouseAction(std::string action, Inputs::MouseButtons button)
{
...
bool InputManager::isKeyActionJustPressed(std::string action) const
{
    ...
    return windowManager->isKeyPressed(it->second) && !windowManager->isKeyRepeated(it->second);
}
...
bool InputManager::isMouseActionJustPressed(std::string action) const
{
    ...
    if (it == actionMouseMap.end())
        return false;

    return windowManager->isMouseButtonPressed(it->second) && !windowManager->isMouseButtonRepeated(it->second);
}
