...
FPSCamera::FPSCamera(const Vector3f &position, const Vector3f &target, const Vector3f &up) : Camera(position, target, up) {
    s = 1.0f;
}

void FPSCamera::handleCursorInput()
{
    ...
    // todo: camera shouldnt need to to track whether the mouse is the first mouse,
    // decouple into window manager
    if (firstMouse)
    {
        lastX = (float)xpos;
        lastY = (float)ypos;
...
void FPSCamera::handleKeyboardInput(float deltaTime)
{
    Application* application = Application::getInstance();
