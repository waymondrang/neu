...
Camera::Camera(const Vector3f &position, const Vector3f &target,
  ...
  pitch = -atan2f(dot(cross(direction, globalDirection),
                      normalize(cross(direction, globalUp))),
                  dot(globalDirection, direction));
  yaw = 0.0f;
...
Camera::Camera() {
  ...
  globalUp = Vector3f(0.0f, 1.0f, 0.0f);
  globalRight = Vector3f(1.0f, 0.0f, 0.0f);
  globalDirection = Vector3f(0.0f, 0.0f, -1.0f);
...
void Camera::updateProjectionMatrix() {
  projectionMatrix[0] =
      1.0f / ((float)s * (float)resolution.x / (float)resolution.y);
