...
#define CHECK_MOUSE_BUTTON_PRESSED(button) \
  glfwGetMouseButton(window, button) == GLFW_PRESS

using std::vector;
using namespace Neu;

WindowManager::WindowManager(Application* context) { this->context = context; }

void WindowManager::initialize(int width, int height, std::string title) {
  this->width = width;
  this->height = height;
...
void WindowManager::endFrame() { glfwSwapBuffers(window); }
...
void WindowManager::closeWindow() { glfwSetWindowShouldClose(window, true); }

void WindowManager::registerFramebufferSizeCallback(
...
//     !inputManager->isMiddleMouseRepeated)
//     {
//         toggleCursor();
//     }
// }

bool WindowManager::isKeyPressed(int key) const { return keyPressed.at(key); }

bool WindowManager::isKeyRepeated(int key) const { return keyRepeated.at(key); }
